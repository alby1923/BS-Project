...
saveRDS(df_filled, file = "df_filled90d.rds")
...
ggplot(na_counts, aes(x = Variable, y = NA_Count)) +
  ...
  geom_hline(yintercept = nrow(df_responses), linetype = "dashed", color = "red") + 
  theme_minimal() 
...
df <- df_responses_filled[,-c(4,17,27:31,34,35,37)]
...
df_opt <- df[-rows_to_remove,]

saveRDS(df_opt,'notNAresponses.rds')
